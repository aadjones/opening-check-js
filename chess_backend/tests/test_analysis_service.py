...
import pytest
from chess_backend.analysis_service import perform_game_analysis
from chess_backend.deviation_result import DeviationResult
# Import the actual lichess_api module to construct mock return values if needed (like Study)
from chess_backend import lichess_api as actual_lichess_api_module
from chess_backend import pgn_utils
import chess.pgn
...
def test_perform_analysis_no_deviation_user_as_white(mocker):
    """
    ...
    mocker.patch('chess_backend.analysis_service.get_last_games_pgn', return_value=recent_game_pgn)

    mock_study_object = actual_lichess_api_module.Study(chapters=[pgn_utils.pgn_string_to_game(repertoire_pgn_white)])
    mocker.patch('chess_backend.analysis_service.lichess_api.Study.fetch_url', return_value=mock_study_object)
...
def test_perform_analysis_no_deviation_user_as_black(mocker):
    """
...
def test_perform_analysis_deviation_found_user_as_white(mocker):
    """
    ...
    mocker.patch('chess_backend.analysis_service.get_last_games_pgn', return_value=recent_game_pgn_deviation)
...
def test_perform_analysis_deviation_found_user_as_black(mocker):
    """
...
def test_perform_analysis_opponent_deviates_user_as_white(mocker):
    """
    ...
    mocker.patch('chess_backend.analysis_service.get_last_games_pgn', return_value=recent_game_pgn_opponent_deviates)
...
def test_perform_analysis_api_call_fails_gracefully(mocker):
    """
    Test that if get_last_games_pgn returns None (simulating an API failure),
    the analysis returns an empty list.
    """
    mocker.patch('chess_backend.analysis_service.get_last_games_pgn', return_value=None)
...
def test_perform_analysis_study_fetch_fails_gracefully(mocker):
    """
    ...
    mocker.patch('chess_backend.analysis_service.get_last_games_pgn', return_value=recent_game_pgn)
    mocker.patch('chess_backend.analysis_service.lichess_api.Study.fetch_url', side_effect=Exception("Simulated fetch failure"))
